...
export default function ServiceOrdersPage() {
  ...
  const handleComplete = async (id: string) => {
    ...
    try {
      await api.patch(`/service-orders/${id}/complete`, {});
      await load();
    } catch (err: unknown) {
  ...
  const f =
    (k: keyof OrderForm) =>
    (e: React.ChangeEvent<HTMLSelectElement> | React.ChangeEvent<HTMLTextAreaElement>) =>
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="max-w-xs truncate px-5 py-3 text-gray-600">{ticketSubject(o.ticketId)}</td>
                  <td className="px-5 py-3 text-gray-500">{techName(o.tecnicoId)}</td>
                  <td className="px-5 py-3">
                    <Badge variant={orderStatusVariant(o.estado)}>{fmt(o.estado)}</Badge>
                  </td>
