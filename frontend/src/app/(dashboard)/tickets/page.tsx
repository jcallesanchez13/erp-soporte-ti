...
export default function TicketsPage() {
  ...
  const handleStatusChange = async () => {
    ...
    try {
      await api.patch(`/tickets/${statusModal.id}/status`, { estado: newStatus });
      await load();
      setStatusModal(null);
    } catch (err: unknown) {
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter((t) => {
      ...
      const matchesStatus = filterStatus === 'TODOS' ? true : t.estado === filterStatus;
      const matchesPriority = filterPriority === 'TODAS' ? true : t.prioridad === filterPriority;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Gestiona incidencias y soporte técnico</p>
        </div>
      ...
      {filteredTickets.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
          <div className="border-b border-gray-100 px-5 py-4">
            <p className="text-sm font-medium text-gray-600">{filteredTickets.length} ticket(s) encontrados</p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredTickets.map((t) => (
                  <tr key={t.id} className="border-b border-gray-50 transition-all duration-200 hover:bg-orange-50/40">
                    <td className="px-5 py-4">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-gray-900">{t.titulo}</span>
                          {t.prioridad === 'CRITICA' && (
                            <span className="rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold uppercase text-red-600">Urgente</span>
                          )}
                        </div>
                        <span className="text-xs text-gray-400">ID: {t.id.slice(0, 8)}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-gray-600">{clientName(t.clienteId)}</td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => { setStatusModal(t); setNewStatus(t.estado); }}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              onChange={(e) => { f('clienteId')(e); setForm((p) => ({ ...p, activoId: '' })); }}
          ...
          <Input label="Título *" value={form.titulo} onChange={f('titulo')} required />
      ...
      {/* DETAILS MODAL */}
      <Modal open={!!detailsTicket} onClose={() => setDetailsTicket(null)} title="Detalle del ticket" maxWidth="lg">
        {detailsTicket && (
          <div className="flex flex-col gap-5">
            <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-bold text-gray-900">{detailsTicket.titulo}</h2>
                  <p className="mt-1 text-sm text-gray-500">Ticket #{detailsTicket.id.slice(0, 8)}</p>
                </div>
                <Badge variant={ticketPriorityVariant(detailsTicket.prioridad)}>{fmt(detailsTicket.prioridad)}</Badge>
              </div>
            </div>
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-gray-100 p-4">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Cliente</p>
                <p className="font-medium text-gray-800">{clientName(detailsTicket.clienteId)}</p>
              </div>
              <div className="rounded-xl border border-gray-100 p-4">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Estado</p>
                <Badge variant={ticketStatusVariant(detailsTicket.estado)}>{fmt(detailsTicket.estado)}</Badge>
              </div>
            </div>
            <div className="rounded-xl border border-gray-100 p-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-400">Descripción</p>
              <p className="text-sm leading-relaxed text-gray-700">{detailsTicket.descripcion || 'Sin descripción'}</p>
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Clock3 className="h-4 w-4" />
              Creado el {formatDate(detailsTicket.createdAt)}
            </div>
          </div>
        )}
      </Modal>
